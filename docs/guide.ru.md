...
Это отдельный эксперимент с более длинным выводом и включённым наблюдением за генерацией. Полная серия ещё не завершена; ускорение и полный паритет для всех 100 строк пока не установлены. Первая попытка исключена из-за подтверждённого сна Mac, Повтор с проверкой сна и питания подготовлен и отложен; сейчас долгие прогоны не запущены. Результаты прежних пяти запросов ниже не заменяют её итогов.
...
bash scripts/benchmark/mtp_series.sh \
  ...
  --label mtp-long2048-awake --output artifacts/series/mtp-long2048-awake
```
...
Ход серии и попытки сохраняются в `artifacts/series/mtp-long2048-awake/manifest.json`, а измерения — в отдельных каталогах `artifacts/runs/`. После остановки продолжите **ту же конфигурацию** с `--resume`:

```bash
bash scripts/benchmark/mtp_series.sh \
  --count 100 --max-new-tokens 2048 \
  --chunk-size 5 --block-size 3 --trace-generation \
  --label mtp-long2048-awake --output artifacts/series/mtp-long2048-awake --resume
```

Продолжение сверяет параметры, хеши измерительного кода, входов и runtime, а также проверяет уже завершённые процессы. Оно не запускает второй экземпляр поверх работающей серии. Незавершённый процесс повторяется целиком в новой попытке; прежние частичные файлы сохраняются, их строки не смешиваются с новой попыткой. Изменение закреплённых входов или кода требует отдельной серии. По умолчанию перед процессом и после него требуются AC Power и отсутствие разряда батареи; нарушение даёт `paused_power`. Сон Mac более 1 секунды обнаруживается по двум монотонным Mach clocks и делает всю попытку блока недопустимой (`paused_sleep`). `--resume` после устранения причины повторяет такой блок; его старые записи сохраняются и не участвуют в сводке. При разряде ниже 20% действует дополнительный порог `paused_battery` (`--min-battery-percent`). Флаг `--allow-battery` явно снимает требование AC, но в публикуемой чистой серии не используется. Проверки питания относятся к границам процесса, а не ко всему времени внутри блока; точная методика и история прерванной попытки — в [протоколе](benchmark-methodology.md).
